...
from xrally_kubernetes.tasks.scenarios.volumes import configmap


class CreateAndDeleteConfigMapVolumeTestCase(test.TestCase):

    def setUp(self):
        super(CreateAndDeleteConfigMapVolumeTestCase, self).setUp()
        context = {
            "iteration": 1,
            "kubernetes": {
                "namespaces": ["ns"],
        ...
        self.scenario = configmap.CreateAndDeletePodWithConfigMapVolume(
            context)
...
class CreateCheckAndDeleteConfigMapVolumeTestCase(test.TestCase):

    def setUp(self):
        super(CreateCheckAndDeleteConfigMapVolumeTestCase, self).setUp()
        context = {
            "iteration": 1,
            "kubernetes": {
                "namespaces": ["ns"],
    ...
    def test_delete_failed(self):
        ...
        self.assertRaises(rest.ApiException, self.scenario.run, "test/image",
                          configmap_data={"test.txt": "test"},
                          subpath="test.txt", mount_path="/opt/check",
                          check_cmd=["ls"])
        self.client.create_pod.assert_called_once_with(
            "test/image",
            name="name",
